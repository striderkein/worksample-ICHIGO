...
const registerCustomer = async (db, req, res) => {
  ...
  insertCustomer.run([name, '🥉', dayjs().format('YYYY-MM-DD'), 0]);

  return res.json({ message: "Customer created!", data: { name } });
};
...
const getCustomerInfo = async (db, req, res) => {
  ...
  const lastYearOrders = db.prepare("SELECT * FROM customers c INNER JOIN orders o WHERE c.name = ? AND c.name = o.customerId AND o.date >= ?");
  lastYearOrders.all([customerId, firstDayOfLastYear], (err, rows) => {
    ...
    const nextRankThreshold = nextRank === '🥈' ? 10000 : 50000;
    // 4. 次のランクに到達するために使用しなければならない金額
    ...
    // ダウングレード判定の閾値
    const downgradeThreshold = currentRank === '🥉' ? 0 : downgradeRank === '🥈' ? 50000 : 10000;
    // 7. ダウングレードを回避するために顧客が今年使う必要のある金額
